...
axes[0, 1].axis('off')
axes[0, 1].plot([0, xmax/2, xmax/2], [ymax/2, ymax/2, 0.65*ymax], color='k')
axes[0, 1].arrow(xmax/2, 0.85*ymax, 0, 0.15*ymax, color='k',
          head_width=2, head_length=3, length_includes_head=True, overhang=0.2)
...
axes[1, 0].plot([xmax/2, xmax/2], [ymax, 2*ymax/3], color='k')
axes[1, 0].arrow(xmax/2, ymax/3, 0, -ymax/3, color='k',
          head_width=2, head_length=3, length_includes_head=True, overhang=0.2)
...
#cp.imshow(cell_raw.coords.rc < cell_raw.coords.r, ax=axes[2, 0], cmap='gray_r')
...
axes[2, 1].plot([xmax/2, xmax/2], [0, 0.2*ymax], color='k')
axes[2, 1].plot([xmax/2, xmax/2], [0.375*ymax, 0.5*ymax], color='k')

axes[2, 1].arrow(xmax/2, ymax/2, -xmax/2, 0, color='k',
          head_width=2, head_length=3, length_includes_head=True, overhang=0.2)
axes[2, 1].axis('off')
...
#cp.imshow(cell_raw.coords.rc, cmap='jet', ax=axes[2, 1])
# print(cell_raw.coords.y_coords)
# axes[2, 1].contourf(cell_raw.coords.x_coords, cell_raw.coords.y_coords, cell_raw.coords.rc, extent=[10, xmax, ymax, 0], cmap='viridis')#, origin='image')
# axes[2, 1].set_xlim(0.5, xmax-0.5)
# axes[2, 1].set_ylim(ymax, 0.5)


# grid[0].set_ylabel("Measured")
# grid[3].set_ylabel("Calculated")
# grid[6].set_ylabel("Final result")
#
# grid[0].set_title('Binary')
# grid[1].set_title('Brightfield')
# grid[2].set_title('Fluorescence')
